    ...
    startBot() {
        this.bot.start(async (ctx) => {
            ...
            if (chatIdNum === ADMIN_CHAT_ID) {
                ...
                try {
                    ...
                    worksheet.addRow([]);
                    worksheet.getCell('A1').value = `📊 Hisobot: Barcha ma'lumotlar (${docs.length} ta yozuv), saralangan: Eski birinchi`;
                    ...
                    await ctx.reply(`✅ Hisobot yuborildi! Jami ${docs.length} ta ma'lumot, saralangan: Eski birinchi (yangi oxirida). Vaqt Toshkent bo'yicha.`);
                }
                catch (error) {
        ...
        this.bot.on('text', async (ctx) => {
            ...
            if (step === 'askName') {
                ...
                return ctx.reply('Jinsingizni tanlang:', telegraf_1.Markup.inlineKeyboard([
            ...
            if (step === 'askBirthDate') {
                ...
                if (!dateRegex.test(birthDateStr)) {
                    return ctx.reply("Iltimos, tug'ilgan kuningizni DD.MM.YYYY formatida kiriting (masalan: 02.10.1999):");
                }
        ...
        this.bot.action(/spec_(.+)/, async (ctx) => {
            ...
            await ctx.answerCbQuery();
            await ctx.reply('Nogironlik guruhini tanlang:', telegraf_1.Markup.inlineKeyboard([
        ...
        this.bot.action(/disability_(.+)/, async (ctx) => {
            ...
            await ctx.answerCbQuery();
            await ctx.reply('Aloqa uchun telefon raqamingizni ulashing:', telegraf_1.Markup.keyboard([telegraf_1.Markup.button.contactRequest('📞 Telefon raqamni ulashish')])
                .oneTime()
                .resize());
        });
